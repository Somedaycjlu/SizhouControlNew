...
u8 AnBT_DMP_MPU6050_Init(void)	
{
	...
	//
	if(!mpu_set_sensors(INV_XYZ_GYRO | INV_XYZ_ACCEL)) AnBT_Uart1_Send_String("P-2,Finish DMP set sensor.",26);		//完成传感去设定
	else AnBT_Uart1_Send_String("P-2,DMP set sensor error.",25);
	//
	if(!mpu_configure_fifo(INV_XYZ_GYRO | INV_XYZ_ACCEL)) AnBT_Uart1_Send_String("P-3,Finish DMP configure fifo.",30);		//完成fifo设置
	else AnBT_Uart1_Send_String("P-3,DMP configure fifo error.",29);
	//
	if(!mpu_set_sample_rate(DEFAULT_MPU_HZ))AnBT_Uart1_Send_String("P-4,Finish DMP set sample rate.",31);			//完成DMP采样率设置
	else AnBT_Uart1_Send_String("P-4,DMP set sample_rate error.",30);
	//
	if(!dmp_load_motion_driver_firmware())	AnBT_Uart1_Send_String("P-5,Finish DMP load motion driver firmware.",43);				//开启DMP功能
	else AnBT_Uart1_Send_String("P-5,DMP load motion driver firmware error.",42);
	//
	if(!dmp_set_orientation(inv_orientation_matrix_to_scalar(gyro_orientation))) AnBT_Uart1_Send_String("P-6,Finish DMP set orientation.",31);		//初始方向设定偏差
	...
	//
	if(!dmp_set_fifo_rate(DEFAULT_MPU_HZ)) AnBT_Uart1_Send_String("P-8,Finish DMP set fifo rate.",29);		//DMP  fifo速度
	else AnBT_Uart1_Send_String("P-8,DMP set fifo rate error.",28);
	//
	run_self_test();
	if(!mpu_set_dmp_state(1)) AnBT_Uart1_Send_String("P-9,Finish DMP set dmp state.",29);				//DMP成功开启
